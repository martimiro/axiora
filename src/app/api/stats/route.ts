...
export async function GET() {
  ...
  try {
    ...
    const ticketsToday = tickets.filter(t => new Date(t.createdAt) >= startOfDay)

    const user = await prisma.user.findUnique({ where: { id: userId } })
    const limits = getPlanLimits(user?.plan ?? 'basic', user?.role)
    ...
    return NextResponse.json({
      ...
      plan: {
        name: user?.plan ?? 'basic',
        maxAgents: limits.maxAgents === Infinity ? null : limits.maxAgents,
        emailsUsed: user?.emailsUsedThisMonth ?? 0,
